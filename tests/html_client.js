...
function voidElements() {
    let template = tmplStr(`<input>   <input>
            <input><input> `, []);
...
function textAndInlineElements() {
    let template = tmplStr(`beasts <span>    tread		</span>     softly <span>    underfoot </span>      .`, []);
...
function textAndBlocks() {
    let template = tmplStr(`beasts <p>    tread		</p>     softly <p>    underfoot </p>      .`, []);
...
function nestedElementsAndText() {
    let template = tmplStr(`<a><label><input type=woofer>bark!</label><img></a>`, []);
...
function document() {
    ...
    let expected = "<!DOCTYPE><html><head></head><body><article>You're a <span>boy kisser</span> aren't you? Click <a>here</a> and go somewhere else.</article><footer></footer></body></html>";
    let html = new ClientHtml();
    let results = html.build(template);
    return assert(expected, results);
}
...
if 2 < 3 {
    ...
    let expected = "<!DOCTYPE><html><head></head><body><article></article><footer></footer></body></html>";
    let html = new ClientHtml();
    let results = html.build(template);
    return assert(expected, results);
}
